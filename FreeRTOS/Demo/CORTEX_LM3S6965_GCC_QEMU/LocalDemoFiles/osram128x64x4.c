...
OSRAM128x64x4StringDraw(const char *pcStr, unsigned long ulX,
                        unsigned long ulY, unsigned char ucLevel, unsigned char onBuffer)
{
        ...
        for(ulIdx1 = 0; ulIdx1 < 3; ulIdx1++)
        {
            ...
            if(ulX < 126)
            {
            	if (!onBuffer)
            	{
            		...
            		unsigned long i = 0;
            		while (i < 8) {
            			OSRAM128x64x4PixelDraw(ulX, ulY + i, pucBuffer[i]>>4, 1);
            			OSRAM128x64x4PixelDraw(ulX + 1, ulY + i, pucBuffer[i] & 0x0F, 1);
            			i++;
            		}
            	}
                ulX += 2;
               ...
               unsigned long ulHeight, unsigned char onBuffer, unsigned char upscale)
{
    ...
    //
    unsigned long startHeight = ulHeight;
    while(ulHeight>0)
    {
    	if (onBuffer) {
			unsigned long w = 0;
			while(w < ulWidth)
			{
				unsigned char color = pucImage[w/2 + (startHeight - ulHeight)*(ulWidth/2)];
				if (w % 2 == 0) {
					color >>= 4;
				}
				else {
...
void OSRAM128x64x4PixelDraw(unsigned long ulX, unsigned long ulY, unsigned char color, unsigned char onBuffer)
{
	if (onBuffer) {
		buffer[ulY*64+ulX/2] |= (ulX%2==0 ? color<<4 : color);
	}else{
		static unsigned char pucBuffer[8];
		...
		OSRAMWriteData(pucBuffer, 1);
	}
}

void OSRAM128x64x4RectangleDraw(unsigned long ulX, unsigned long ulY, unsigned long sizeX, unsigned long sizeY, unsigned char color, unsigned char onBuffer)
{
	if (onBuffer) {
		unsigned long initSizeX = sizeX;

		while (sizeY > 0)
		{
			while (sizeX > 0) {
				OSRAM128x64x4PixelDraw(ulX + sizeX - 1, ulY + sizeY - 1, color, 1);
				sizeX--;
			}
			sizeY--;
			sizeX = initSizeX;
		...
		pucBuffer[0] = 0x15;
		pucBuffer[1] = ulX / 2;
		pucBuffer[2] = (ulX + sizeX - 2) / 2 + (ulX + sizeX - 2) % 2;
		OSRAMWriteCommand(pucBuffer, 3);
		pucBuffer[0] = 0x75;
		pucBuffer[1] = ulY;
		pucBuffer[2] = ulY + sizeY - 1;
		OSRAMWriteCommand(pucBuffer, 3);

		OSRAMWriteCommand(g_pucOSRAM128x64x4HorizontalInc,
							  sizeof(g_pucOSRAM128x64x4HorizontalInc));

		pucBuffer[0] = color<<4 | color;
		pucBuffer[1] = color<<4;
		pucBuffer[2] = color;

		unsigned long actualXSize = sizeX/2 + sizeX%2 + ((ulX % 2 != 0 && (ulX + sizeX) % 2 != 0) ? 1 : 0);

		while (sizeY > 0)
		{
			unsigned long counterXSize = actualXSize;
			while (counterXSize > 0)
			{
				if (counterXSize == 1 && (ulX + sizeX - 2) % 2 != 0)
					OSRAMWriteData(pucBuffer+1, 1);
				else if (counterXSize == actualXSize && ulX % 2 != 0)
					OSRAMWriteData(pucBuffer+2, 1);
				else
					OSRAMWriteData(pucBuffer, 1);
				counterXSize--;
			}
...
OSRAM128x64x4Init(unsigned long ulFrequency)
{
    ...
    //
    GPIOPinTypeSSI(GPIO_PORTA_BASE, GPIO_PIN_2 | GPIO_PIN_3 | GPIO_PIN_5 | GPIO_PIN_4);
